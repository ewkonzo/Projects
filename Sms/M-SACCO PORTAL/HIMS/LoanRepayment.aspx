<%@ Page Title="" Language="C#" MasterPageFile="~/Home.Master" AutoEventWireup="true" CodeBehind="LoanRepayment.aspx.cs" Inherits="HIMS.LoanRepayment" %>
...
<div class="style1">
<h6 class="style2" style="text-transform:uppercase;"> <b> LOAN REPAYMENT </b> </h6>
</div>
...
<thead>
 ...
 <%
     string path = "Withdrawals.aspx?action=Withdrawals&Show=False&", pagination = "";
...
<%
    using (SqlConnection mConn = new SqlConnection(CommonUtilities.ConnectionString))
    {
        ...
        //Shares Capital
        string SQL = "SELECT COUNT([Entry No]) AS Num FROM [Mobile Loan Repayments] WHERE [Corporate No]=" + "'" + CorporateNo + "'";
        string SQL_2 = "";
    ...
    <th style="background-color:#64B044;color:#000">Document No </th>
     <th style="background-color:#64B044;color:#000">Comment</th>
...
<tbody>
       ...
       <%
            ...
            else
            {
                ...
                SQL_2 = "WITH My_W AS ( SELECT [Entry No],Status,[Account No],[LoanType] ,[Telephone No],Amount,[Comments],[SESSION_ID],[Sent To Journal],[Transaction Date], " +
                       " ROW_NUMBER() OVER(ORDER BY [Entry No] DESC) AS row_number   FROM  [Mobile Loan Repayments] WHERE [Corporate No]=" + "'" + CorporateNo + "') " +
                " SELECT * FROM My_W WHERE row_number BETWEEN @start AND @limit+@start ORDER BY [Entry No] DESC";
            }
            string Status = "", AccountNo = "", AccountName = "", TelephoneNo = "", MPESAReceiptNo = "", SentToJournal = "", Comments="";
            DateTime Datetime;
         ...
         <td style="background-color:#FFFFFF"><%=Status%></td>
           ...
           <td style="background-color:#FFFFFF"><%=AccountNo%></td>
            ...
            <td style="background-color:#FFFFFF"><%=MPESAReceiptNo%></td>
             <td style="background-color:#FFFFFF"><%=Comments%></td>
