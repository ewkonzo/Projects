<%@ Page Title="" Language="C#" MasterPageFile="~/Home.Master" AutoEventWireup="true" CodeBehind="VirtualRegistration.aspx.cs" Inherits="HIMS.VirtualRegistration" %>
...
<div class="style1">
<h6 class="style2" style="text-transform:uppercase;"> <b> VIRTUAL REGISTRATIONS </b> </h6>
</div>
...
<thead>
 ...
 <%
     string path = "VirtualRegistration.aspx?action=Withdrawals&Show=False&", pagination = "";
...
<%
    using (SqlConnection mConn = new SqlConnection(CommonUtilities.ConnectionString))
    {
        ...
        //Shares Capital
        string SQL = "SELECT COUNT([Entry No]) AS Num FROM [Virtual Registrations] WHERE [Corporate No]=" + "'" + CorporateNo + "'";
        string SQL_2 = "";
    ...
    <th style="background-color:#64B044;color:#000">Account Name</th>
    <th style="background-color:#64B044;color:#000">Telephone No</th>
    <th style="background-color:#64B044;color:#000">Occupation</th>
    <th style="background-color:#64B044;color:#000">Place of Birth </th>
     <th style="background-color:#64B044;color:#000">Sent To IPRS</th>
...
<tbody>
       ...
       <%
            ...
            else
            {
                ...
                SQL_2 = "WITH My_W AS ( SELECT [Entry No],Status,[IDNum],[Surname] ,[Othernames],[MSISDN],[Occupation],[Place_Birth],[Sent To IPRS],[Sent To Journal],[Registration Date], " +
                     " ROW_NUMBER() OVER(ORDER BY [Entry No] DESC) AS row_number   FROM  [Virtual Registrations] WHERE [Corporate No]=" + "'" + CorporateNo + "') " +
              " SELECT * FROM My_W WHERE row_number BETWEEN @start AND @limit+@start ORDER BY [Entry No] DESC";
            }
            string Status = "", AccountNo = "", AccountName = "", TelephoneNo = "", MPESAReceiptNo = "", SentToJournal = "", Place_Birth = "", Occupation = "", SentToIPRS="";
            DateTime Datetime;
            ...
            while (dr.Read())
            {
                ...
                AccountNo = dr["IDNum"].ToString();
                AccountName = dr["Surname"].ToString() + " " + dr["Othernames"].ToString();
                TelephoneNo = dr["MSISDN"].ToString();
                Occupation = dr["Occupation"].ToString();
                Place_Birth = dr["Place_Birth"].ToString();
                SentToIPRS = dr["Sent To IPRS"].ToString();
                SentToJournal = dr["Sent To Journal"].ToString();
                Datetime = Convert.ToDateTime(dr["Registration Date"].ToString());
            ...
            <td style="background-color:#E3EAEB"><%=TelephoneNo%></td>
             <td style="background-color:#E3EAEB"><%=Occupation%></td>
            <td style="background-color:#E3EAEB"><%=Place_Birth%></td>
              <td style="background-color:#E3EAEB"><%=SentToIPRS%></td>
         ...
         <td style="background-color:#FFFFFF"><%=Status%></td>
           ...
           <td style="background-color:#FFFFFF"><%=AccountNo%></td>
            ...
            <td style="background-color:#FFFFFF"><%=Occupation%></td>
             <td style="background-color:#FFFFFF"> <%=Place_Birth%></td>
             <td style="background-color:#FFFFFF"> <%=SentToIPRS%></td>
