<%@ Page Title="" Language="C#" MasterPageFile="~/Home.Master" AutoEventWireup="true" CodeBehind="smstopupmobile.aspx.cs" Inherits="HIMS.smstopupmobile" %>
...
<div class="style1">
<h6 class="style2" style="text-transform:uppercase;"> <b> MOBILE SMS TOPUP </b> </h6>
</div>

<li>
	<a href="smstopupmobilenew.aspx?action=mobilesmstopup&Show=False">
...
<thead>
 ...
 <%
     string path = "smstopupmobile.aspx?action=mobilesmstopup&Show=False&", pagination = "";
...
<%
    using (SqlConnection mConn = new SqlConnection(CommonUtilities.ConnectionString))
    {
        ...
        //string SQL = "SELECT [Id],[CorporateNo],[SmsCount],[Datetime],[Comments],[UserID] FROM [Sacco].[dbo].[MSacco TopUp] ";
        string SQL = "SELECT COUNT([Id]) AS Num FROM [Sacco].[dbo].[MSacco TopUp] ";
        string SQL_2 = "";
...
<tbody>
       ...
       <%
            ...
            {                
                SQL_2 = "WITH My_W AS ( " +
                    "SELECT a.[Id],a.[CorporateNo][Client Code],b.[Sacco Name 1] [Client Name],ISNULL(a.[SmsCount],0)[SmsCount]" +
		                ",a.[Datetime][Date],ISNULL(a.[Comments],'')[Comments],ISNULL(a.[UserID],'')[UserID] " +
		                ",ROW_NUMBER()OVER(ORDER BY a.[Id] DESC) AS ROW_NUMBER " +
                    "FROM [Sacco].[dbo].[MSacco TopUp] a " +
                    "INNER JOIN [Sacco].[dbo].[Source Information] b ON a.[CorporateNo]=b.[Corporate No]) " + 
            ...
            while (dr.Read())
            {
                ...
                date =DateTime.Parse (dr["Date"].ToString()).ToString("yyyy-MM-dd HH:mm:ss");
                smscount = dr["SmsCount"].ToString();
                ...
                if (i % 2 == 0)
                {
                 %>
                    ...
                    <td style="background-color:#E3EAEB; text-align:right;"><%=smscount%></td> 
