<%@ Page Title="" Language="C#" MasterPageFile="~/Home.Master" AutoEventWireup="true" CodeBehind="Withdrawals.aspx.cs" Inherits="HIMS.Dashboard" %>
...
<div class="style1">
<h6 class="style2" style="text-transform:uppercase;"> <b> WITHDRAWALS </b> </h6>
</div>
...
<thead>
 ...
 <%
     ...
     {
         path = "Withdrawals.aspx?action=Withdrawals&Show=False&Search_Pin_No=" + Search_PhoneNo + "&";
     }
     else
     {
         path = "Withdrawals.aspx?action=Withdrawals&Show=False&";
     }
...
<%
    using (SqlConnection mConn = new SqlConnection(CommonUtilities.ConnectionString))
    {
        ...
        if (Search_PhoneNo.Trim() != "")
        {
            SQL = "SELECT COUNT([Entry No]) AS Num FROM [Mobile Withdrawals] WHERE [Corporate No]=" + "'" + CorporateNo + "' AND [Telephone No] LIKE '%" + Search_PhoneNo + "%'";
        }
        else
        {
            SQL = "SELECT COUNT([Entry No]) AS Num FROM [Mobile Withdrawals]WHERE [Corporate No]=" + "'" + CorporateNo + "'";
        }
        string SQL_2 = "";
    ...
    <th style="background-color:#64B044;color:#000">M-PESA Receipt No </th>
...
<tbody>
       ...
       <%
            ...
            if (Search_PhoneNo.Trim() != "")
            {
                SQL_2 = "WITH My_W AS ( SELECT [Entry No],Status,[Account No],[Account Name] ,[Telephone No],Amount,[MPESA Float Amount],[MPESA Receipt No],[Sent To Journal],Datetime, " +
                              " ROW_NUMBER() OVER(ORDER BY [Entry No] DESC) AS row_number   FROM  [Mobile Withdrawals] WHERE [Telephone No] LIKE '%"+Search_PhoneNo+"%' AND [Corporate No]=" + "'" + CorporateNo + "') " +
            ...
            else
            {
                ...
                SQL_2 = "WITH My_W AS ( SELECT [Entry No],Status,[Account No],[Account Name] ,[Telephone No],Amount,[MPESA Float Amount],[MPESA Receipt No],[Sent To Journal],Datetime, " +
                       " ROW_NUMBER() OVER(ORDER BY [Entry No] DESC) AS row_number   FROM  [Mobile Withdrawals] WHERE [Corporate No]=" + "'" + CorporateNo + "') " +
                " SELECT * FROM My_W WHERE row_number BETWEEN @start AND @limit+@start ORDER BY [Entry No] DESC";
            ...
            while (dr.Read())
            {
                ...
                Amount = Math.Round(Convert.ToDouble(dr["Amount"].ToString()), 2);
                double MPESAFloatAmount=0;
                if (string.IsNullOrEmpty(dr["MPESA Float Amount"].ToString()))
                {
                    MPESAFloatAmount = 0;
                }
                else {
                    MPESAFloatAmount = Math.Round(Convert.ToDouble(dr["MPESA Float Amount"].ToString()), 2);
                }
                    
               // MpesaFloat = Math.Round(Convert.ToDouble(dr["MPESA Float Amount"].ToString()), 2);
                MPESAReceiptNo = dr["MPESA Receipt No"].ToString();
