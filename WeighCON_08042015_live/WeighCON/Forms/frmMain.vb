...
Public Class frmMain
    ...
    'Sub readInputs()
    '    Try
    '        myCONN.AccDownload("SELECT * FROM ITEMMASTER WHERE NewBarCode='" & Me.txtBarcodeInput.Text & "'")
    ...
    '            theoreticalmass = myreader("Theoretical_Mass")
    '            Me.txtPermittedLowerTol.Text = myreader("Permitted_Tol_Lower_%")
    '            Me.txtPermittedUpperTol.Text = myreader("Permitted_Tol_Upper_%")
    '            upperpercent = IIf(myreader("Permitted_Tol_Upper_%") Is Nothing, 0, myreader("Permitted_Tol_Upper_%"))
    '            lowerpercent = IIf(myreader("Permitted_Tol_Lower_%") Is Nothing, 0, myreader("Permitted_Tol_Lower_%"))
    ...
    '        txtPermittedUpperMass.Text = ((upperpercent + 100) / 100) * theoreticalmass
    '        txtPermittedLowerMass.Text = ((100 - lowerpercent) / 100) * theoreticalmass

    '        'myCONN.AccDownload("Select top(" & CInt(Settings.Averagecounter) & ") avg(mass) from itemlog where barcode = '" & txtBarcodeInput.Text & "' and status = 'OK' and Barcode <> 'NOREAD'")
    '        'If myCONN.Accrec.HasRows Then
    '        '    myCONN.Accrec.Read()
    '        '    Movingmass = myCONN.Accrec.Item(0)
    '        '    txtcurrentavgmass.Text = Movingmass

    '        '    txtPermittedUpperMass.Text = ((My.Settings.Weightvariance + 100) / 100) * Movingmass
    '        '    txtPermittedLowerMass.Text = ((100 - My.Settings.Weightvariance) / 100) * Movingmass
    '        'End If
    '    Catch ex As Exception
    '        log(ex.Message)
    '        log(ex.StackTrace)
    ...
    Sub readInputs()
        Try
            myCONN.AccDownload("SELECT * FROM ITEMMASTER WHERE NewBarCode='" & Me.txtBarcodeInput.Text.Trim() & "'")
            Dim myreader As SqlClient.SqlDataReader
            myreader = myCONN.Accrec
            If myreader.HasRows Then
                myreader.Read()
