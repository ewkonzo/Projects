...
namespace Monitor
{
    ...
    {
        public static Collections.Collection_Service col = new Collections.Collection_Service();
        static System.Net.NetworkCredential cd;

        public collections()
        {

        }
        public static List<Collections.Collection> getcollections()
        {
            string path = HostingEnvironment.MapPath("~/Settings.txt");
         
          s.getsettings(path);
           cd = new System.Net.NetworkCredential(s.user, s.pass, s.domain);
           col.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Collection", s.server, s.Companyname, s.Instance, s.Port);
            col.PreAuthenticate = true;
            col.Credentials = cd;
            var c = col.ReadMultiple(new Collections.Collection_Filter[] { }, null, 5000).ToList();
