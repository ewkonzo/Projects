...
namespace Rice
{
    class Scripting
    {
        ...
        {
            String dbName = rice.setting.database; // database name  
            ServerConnection sc = new ServerConnection();
            sc.LoginSecure = rice.setting.IntegratedSecurity;
            sc.DatabaseName = dbName;
            sc.ServerInstance =string.Format(@"{0}.\{1}",rice.setting.Server, rice.setting.Instance);
            if (!sc.LoginSecure)
            {
                sc.Login = rice.setting.Username;
                sc.Password = rice.setting.pass;
            }
           ...
           scrp.Options.WithDependencies = true;
            ...
            System.IO.StreamWriter file = new System.IO.StreamWriter(@"Script.txt");
