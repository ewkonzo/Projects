...
public class login extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                {
                    Log.i("login", a.Password);
                    Log.i("login", pass.getText().toString());
                    Toast.makeText(getApplicationContext(), "Invalid username or password", Toast.LENGTH_LONG).show();
                    return;
                }
                CurrentAgent = a;
    ...
    private class Getconstituencies extends AsyncTask<Void, String, List<Constituency>> {
        @Override
        protected void onPreExecute() {
        ...
        @Override
        protected List<Constituency> doInBackground(Void... params) {
            publishProgress("Getting Constituencies");
            List<Constituency> results = null;
            String result = null;
            try {
                Gson g = new Gson();
                result = JsonParser.postjson("Constituency", null, null);
                Type localType = new TypeToken<List<Constituency>>() {
                }.getType();
                results = new Gson().fromJson(result, localType);
                if (results != null) {
                    try {
                     //   publishProgress("Updating Constituency");
                        for (Constituency f : results
                                ) {
                            db.inserconstituency(f);
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                ...
                publishProgress("Unable to get Constituency");
                e.printStackTrace();
            }
            return results;
        }
        ...
        @Override
        protected void onPostExecute(List<Constituency> res) {
            try {
                if (res != null)
                    Toast.makeText(getApplicationContext(), "Constituency updated", Toast.LENGTH_LONG).show();
