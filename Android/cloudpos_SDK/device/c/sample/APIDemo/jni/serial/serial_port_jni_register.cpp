...
{
	int nCount = 0;
	JNINativeMethod* pMethods = serial_port_get_methods(&nCount);

	jint nResult = register_native_methods(env,	serial_port_get_class_name(),	pMethods, nCount);
	//	+update by pengli
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	{
		hal_sys_info("serial port JNI_OnLoad(), failed in GetEnv()");
		return -1;
	}
	assert(env != NULL);
