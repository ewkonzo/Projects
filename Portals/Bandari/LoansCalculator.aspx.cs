...
namespace Bandari_Sacco
{
    public partial class LoansCalculator : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Global.mBranch.Delcalc(Session["Member_No"].ToString());
            ...
            else
            {
                try
                {
                    ...
                    for (int i = 1; i <= Convert.ToInt32(this.ddlRepaymentPeriod.SelectedItem.Text); i++)
                    {
                        ...
                        if (this.IsAmortisedLoan(this.ddlLoanTypes.SelectedValue))
                        {
                            loan_Calc.Online_Loan_Calc lcc = new loan_Calc.Online_Loan_Calc();

                            lcc.User_ID = Member_No_;
                            lcc.Month = i;
                            lcc.Principle_Amount = MyClass.FormatNumber(loanAmount);
                            lcc.Loan_Repayment = MyClass.FormatNumber(pmt);
                            lcc.Interest = MyClass.FormatNumber(interest * loanAmount);
                            lcc.Total_Deduction = MyClass.FormatNumber(loanRepayment);
                            lcc.Loan_Balance = MyClass.FormatNumber(loanBalance);
                            lcc.Period = String.Format("{0}-{1}", new DateTimeFormatInfo().GetAbbreviatedMonthName(date.Month), date.Year);

                            Global.loan_Calc_Service.Create(ref lcc);
                        ...
                        {
                            loan_Calc.Online_Loan_Calc lac = new loan_Calc.Online_Loan_Calc();

                            lac.User_ID = Member_No_;
                            lac.Month = i;
                            lac.Principle_Amount = MyClass.FormatNumber(loanAmount);
                            lac.Loan_Repayment = MyClass.FormatNumber(loanRepayment);
                            lac.Interest = MyClass.FormatNumber(interest * loanAmount);
                            lac.Total_Deduction = MyClass.FormatNumber(pmt);
                            lac.Loan_Balance = MyClass.FormatNumber(loanBalance);
                            lac.Period = String.Format("{0}-{1}", new DateTimeFormatInfo().GetAbbreviatedMonthName(date.Month), date.Year);

                            Global.loan_Calc_Service.Create(ref lac);
                    ...
                    loan_Calc.Online_Loan_Calc lc = new loan_Calc.Online_Loan_Calc();

                    lc.User_ID = Member_No_;
                    lc.Month = lineNo;
                
                    lc.Loan_Repayment = MyClass.FormatNumber(this.txtLoanAmount.Text);
                    lc.Interest = MyClass.FormatNumber(totalInterest);
                    lc.Total_Deduction = MyClass.FormatNumber(totalInterest + Convert.ToDouble(this.txtLoanAmount.Text));
                   
                    lc.Period = "TOTALS";

                    Global.loan_Calc_Service.Create(ref lc);
        ...
        protected void LoadLoanTypes()
        {
            try
            {
              ...
              var pp =   Global.products_Service.ReadMultiple(new loanproducts.Loan_Products_Filter[] { }, null, 0);
                if (pp.Count()>0)
                    foreach (var p in pp)
                    {
                        if (p.Product_Description.ToString().Trim() != "")
                        {
                            li = new ListItem(
                                p.Product_Description.ToString(),
        ...
        protected int MaximumRepayment(string loanType)
        {
            ...
            try
            {
                var lp = Global.products_Service.Read(loanType);
                if (lp != null) {
                    i = lp.Maximum_Repayment_Period_M;
                }
        ...
        protected bool IsAmortisedLoan(string loanType)
        {
            ...
            try
            {
                ...
                if (lp != null)
                {
                    b = lp.Interest_Rate_Type == loanproducts.Interest_Rate_Type.Amortised;
                }
        ...
        private string NormalOutstandingLoanNo(string Member_No_)
        {
            ...
            try
            {
                var lb = Global.loan_Service.ReadMultiple(new Mobile_Loan.Mobile_Loan_Filter[] { new Mobile_Loan.Mobile_Loan_Filter { Criteria = Member_No_, Field = Mobile_Loan.Mobile_Loan_Fields.Member_No },  new Mobile_Loan.Mobile_Loan_Filter { Criteria = "A26", Field = Mobile_Loan.Mobile_Loan_Fields.Loan_Product } },null, 0 ).FirstOrDefault();
                if (lb != null)
                    s = lb.Loan_No;
                            }
            catch (Exception ex)
        ...
        #endregion
        protected void ddlRepaymentPeriod_SelectedIndexChanged(object sender, EventArgs e)
        {
