...
namespace AGENCY
{
    ...
    { 
        ...
        public static PostAgent.Post Post = new PostAgent.Post();
        Sms sms = new Sms();
        public Agency()
        {
            path = Server.MapPath("~/settings.txt");
            s = s.getsettings(path);
         
            cd = new System.Net.NetworkCredential(s.Username, s.pass, s.domain);
            CUtilities.LogEntryOnFile(String.Format("http://{0}:{3}/{2}/WS/{1}/Page/AgentTransactions", s.Serverip, s.Companyname,
                s.Instance, s.Port));
            Transaction.Agtranservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/AgentTransactions", s.Serverip, s.Companyname,
                s.Instance, s.Port);
            Transaction.Agtranservice.PreAuthenticate = true;
            Transaction.Agtranservice.Credentials = cd;

            Post.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Codeunit/Post", s.Serverip, s.Companyname,
                   s.Instance, s.Port);
            Post.PreAuthenticate = true;
            Post.Credentials = cd;

            Account.Accservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Vendors", s.Serverip, s.Companyname,
                  s.Instance, s.Port);
            Account.Accservice.PreAuthenticate = true;
            Account.Accservice.Credentials = cd;
            Account.accdetservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/CustomerDetails", s.Serverip, s.Companyname,
                            s.Instance, s.Port);
            Account.accdetservice.PreAuthenticate = true;
            Account.accdetservice.Credentials = cd;

            Account.Custservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Customers", s.Serverip, s.Companyname,
                            s.Instance, s.Port);
            Account.Custservice.PreAuthenticate = true;
            Account.Custservice.Credentials = cd;

            Agent.agentservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/AgentApplications", s.Serverip, s.Companyname,
                            s.Instance, s.Port);
            Agent.agentservice.PreAuthenticate = true;
            Agent.agentservice.Credentials = cd;


            Loans.loanservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/Loans", s.Serverip, s.Companyname,
                           s.Instance, s.Port);
            Loans.loanservice.PreAuthenticate = true;
            Loans.loanservice.Credentials = cd;

            Loans.productservice.Url = String.Format("http://{0}:{3}/{2}/WS/{1}/Page/LoanProducts", s.Serverip, s.Companyname,
                           s.Instance, s.Port);
            Loans.productservice.PreAuthenticate = true;
            Loans.productservice.Credentials = cd;
        }
        [WebMethod]
                ...
                switch (trans.transactiontype)
                {
                   ...
                   case Transaction.Transtype.LoanRepayment:
                    ...
                    case Transaction.Transtype.Withdrawal:
                        ...
                        break;
                    case Transaction.Transtype.Sharedeposit:case Transaction.Transtype.Share_Variation:
 member.accounts = Account.Memberlist(member.id_no);
                        break;
                    case Transaction.Transtype.Loan_Applications:
                        member.accounts = Account.Memberlist(member.id_no);
                        trans.products = Loans.loanproducts();
                        break;
                    default:
                        ...
                        case Transaction.Status.Pending:
                            {
                                ...
                                try
                                {
                                    ...
                                    switch (tr.transactiontype)
                                    {
                                        ...
                                        case Transaction.Transtype.Share_Variation:
                                ...
                                {
                                    CUtilities.LogEntryOnFile("errrors");
                                    CUtilities.LogEntryOnFile(ex.Message);
                                    CUtilities.LogEntryOnFile(ex.StackTrace);
                                ...
                                if (t != null)
                                {
                                    try
                                    {
                                        ...
                                        //    {
                                                ...
                                                int agno =Convert.ToInt32( tr.agent.agent_code.Replace("AGN", ""));
                                                ...
                                                AgencyTransactions.AgentTransactions trans = new AgencyTransactions.AgentTransactions();
                                                trans.Document_No = tr.reference;
                                                ...
                                                trans.Transaction_Date = DateTime.Now.Date;
                                                trans.Transaction_DateSpecified = true;
                                                trans.Telephone = tr.Telephone_No;
                                                trans.Account_No = (tr.account_1!=null? tr.account_1.Account_No:"");
                                                trans.Account_No_2 =(tr.account_2!=null? tr.account_2.Account_No:"");
                                                trans.Transaction_Type = (AgencyTransactions.Transaction_Type)tr.transactiontype;

                                                trans.Transaction_TypeSpecified = true;
                                                trans.Agent_Code = tr.agent.agent_code;
                                                trans.Amount = (decimal)tr.amount;
                                                trans.AmountSpecified = true;
                                                trans.Description = Transaction.GetDescription(tr.transactiontype);
                                                trans.Id_No = (tr.member_1 == null ? "" : tr.member_1.id_no);
                                                trans.Loan_No =(tr.loan_no!=null? tr.loan_no.Loan_No:"");
                                                trans.Member_No =(tr.society!= null? tr.society.memberid:"");

                                                Transaction.Agtranservice.Create(ref trans);
                                                try {
                                                    Post.CallPost(trans.Document_No);
                                                }
                                                catch (Exception ex) { CUtilities.LogEntryOnFile(ex.Message); }
                                                tr.status = Transaction.Status.Successful;
       ...
       try
            {
                balance = (double)Post.Tcharges((decimal)Amount, (int)t);          
            }
            catch (Exception ex)
            {
                CUtilities.LogEntryOnFile(ex.Message);
