...
namespace Server
{
       ...
       // Database cnt = new Database();
        ...
        private void AtmRequests()
        {
            ...
            try
            {
                ...
                Console.Title =
                    "[Copyright Iansoft Kenya " + DateTime.Now.Year +" © ATM Bridge®] " +
        ...
        private string[,] DecodeRequest(string clientRequest, bool request)
        {
            ...
            try
            {
                ...
                string temp = "";
                int i = 0;
                temp = Hexabinary.binary(clientRequest.Substring(8, 32));
        ...
        protected string Encode(string clientRequest, string[,] fieldsPresent)
        {
            ...
            try
            {
                ...
                s = Hexabinary.binary(clientRequest.Substring(8, 32));
                //for (int i2 = 0; i2 < 32; i2++)
                //{
                //    Database cnt = new Database(); System.Data.SqlClient.SqlDataReader dr = cnt.ReadDB(
                //        "select Binary from [" + company + "$Hexa Binary]" +
        ...
        private int FieldLength(int fieldNumber)
        {
            ...
            try
            {
                var iso = IsoElements.getisoelement();
                var f = iso.FirstOrDefault(o => o.Data_Element == fieldNumber);
                if (f != null && f.Length.ToString() != null)
                    i = f.Length;
        ...
        private bool FieldIsVariable(int fieldNumber)
        {
            ...
            {
                var iso = IsoElements.getisoelement();
                var i = iso.FirstOrDefault(o => o.Data_Element == fieldNumber);
                if (i != null)
                    b = i.Variable_Field;
        ...
        private int VariableFieldLength(int fieldNumber)
        {
            ...
            try
            {
                var iso = IsoElements.getisoelement();
                var f = iso.FirstOrDefault(o => o.Data_Element == fieldNumber);
                if (f != null && f.Variable_Field_Length.ToString() !=null)
                    i = f.Variable_Field_Length;
        ...
        private string ResponseBalance(string request, string oldBinary, enBalance balanceType, string[,] fieldsPresent)
        {
            ...
            try
            {
                ...
                temp = Hexabinary.hexadecimal(oldBinary);// "";
                ...
                for (int i = 0; i < fieldsPresent.GetUpperBound(0); i++)
                {
                    ...
                    if (Convert.ToInt32(fieldsPresent[i, 0]) > 54 && !f54)//insert field 54 first
                    {
                        ...
                        {
                            Bridge.Atm_Logs.Atm_Logs log = new Bridge.Atm_Logs.Atm_Logs();
                            log.Date_Time = DateTime.Now;
                            log.Date_TimeSpecified = true;
                            log.Account_No = ATM.CustomerNo(this.atmAccountNo.ToUpper());
                            log.Amount = (decimal)this.atmAmount;
                            log.AmountSpecified = true;
                            log.ATM_No = this.atmUnitID;
                            log.ATM_Location = this.atmLocation;
                            log.Transaction_Type = (Bridge.Atm_Logs.Transaction_Type)balanceType;
                            log.Transaction_TypeSpecified = true;
                            log.Return_Code = (Bridge.Atm_Logs.Return_Code)(Convert.ToInt32(successCode));
                            log.Return_CodeSpecified = true;
                            log.Trace_ID = this.atmTraceID;
                            log.Card_No = this.atmAccountNo;
                            log.Withdrawal_Location = this.atmLocation;
                            log.Reference_No = RetrievalReferenceNo;

                            Program.logs_Service.Create(ref log);
        ...
        private string ResponseMiniStatement(string request, string oldBinary, string[,] fieldsPresent)
        {
            ...
            try
            {
                ...
                var no = this.VendorNo(accountNo);

                var min = Program.entries_Service.ReadMultiple(new Bridge.Account_Entries.Account_Entries_Filter[] { new Bridge.Account_Entries.Account_Entries_Filter { Criteria =no, Field = Bridge.Account_Entries.Account_Entries_Fields.Vendor_No } }, null, 8).Take(8);
 ...
 //if (dr2.HasRows)
                if (min.Count()>0)
                {
                    ...
                    foreach (var mm in min)
                   
                    {
                        temp3 = "";

                        int amountTemp = (int)mm.Amount;// Convert.ToInt32(dr2["Amount"]);
                        ...
                        description = mm.Desc;// this.TransactionDescription(                            Convert.ToInt32(dr2["Vendor Ledger Entry No_"]));

                        //get transaction type
                        if (mm.Amount < 0) // Convert.ToDouble(dr2["Amount"])
                            transactionType = "21";
                        else
                            transactionType = "01";
                        ...
                        miniStatement = Logs.FormatDateWithoutSymbols(mm.Posting_Date) .ToString() +//Convert.ToDateTime(dr2["Posting Date"]) )
               ...
               // dr2.Close();
                ...
                log.ATM_No = this.atmUnitID;
                log.ATM_Location = this.atmLocation;
                log.Transaction_Type = (Bridge.Atm_Logs.Transaction_Type)this.TransactionTypeX;
                log.Transaction_TypeSpecified = true;
                log.Return_Code = (Bridge.Atm_Logs.Return_Code.Transaction_Succeded);
                log.Return_CodeSpecified = true;
                log.Trace_ID = this.atmTraceID;
                log.Card_No = this.atmAccountNo;
                log.Withdrawal_Location = this.atmLocation;
